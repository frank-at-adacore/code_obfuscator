private package Obfuscate.Names is
   ...
   function Last_Dot
     ...
     Post =>
      ((Last_Dot'Result in Str'Range and then Str (Last_Dot'Result) = '.'
        and then
        (for all C of Str (Last_Dot'Result + 1 .. Str'Last) => C /= '.'))
   ...
   function Name_Part
     ...
     Pre  =>
      Str'Length <= Max_Qualified_Name_Length and Str'First > Integer'First and
      Str'Last < Integer'Last,
     Post =>
      Name_Part'Result'Length <= Max_Qualified_Name_Length and
      (if Last_Dot (Str) in Str'Range then
         Name_Part'Result = Str (Last_Dot (Str) + 1 .. Str'Last)
       else Name_Part'Result = Str);

   procedure Add_Name (Qualified_Name : Wide_Wide_String) with
     Pre =>
      Qualified_Name'Length <= Max_Qualified_Name_Length and
